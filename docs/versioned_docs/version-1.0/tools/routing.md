...
`Route::laravext($uri = null, $route_group_attributes = [], $root_view = null, ...$parameters)`
...
- `parameters`: is a variadic parameter that can be used to pass one or more of the following parameters:
  - `route_registration_method`: the method that will be used to register this route. If this is not defined, the route will be declared as a `match(['get', 'head'])`, but you can set it to any other method. For additional details on why this exists, check the [Configuration/Route Registration Method](/configuration.md?id=route_registration_method#route-registration-method-route_registration_method) section.
